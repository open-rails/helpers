...
"Contagious Interview" / PolinRider class) that infected contractor machines and
appended an obfuscated dropper to build-config files in every repo they could
write to, recurring every few weeks from 2025-11 to 2026-05 (`vite.config.js`,
`webpack.config.js`, `frontend/tailwind.config.ts`, `frontend/postcss.config.cjs`,
and `frontend/scripts/setup-husky.mjs`).
...
The workflow checks out the caller, checks out helpers at the pinned SHA (so the
scanner and its fixtures match the pin), runs `--self-test` and scans the
caller's whole tracked tree.

## Run it locally

Bash 4.4+ and GNU grep with PCRE. From the repository to scan, or with
`--root <dir>`:

```bash
S=path/to/helpers/scripts/scan-injected-code.sh
$S                                      # whole tracked tree (full audit)
$S --all --include-untracked
$S --staged                             # staged blobs (pre-commit hook)
$S --range origin/master...HEAD
$S --files path/to/file                 # named paths, no exclusions
$S --self-test                          # fixtures, then the current repository
```

Exit code is non-zero if anything is found. A pre-commit hook is one line:
`exec path/to/scan-injected-code.sh --staged`.
...
- `scripts/scan-injected-code.sh` and `docs/injection-scan.md` are exempt from
  R2-R5 and R12 (both quote the signatures they describe) but are still subject
  to R1.
...
- Rules 12 and 13 are the only ones that look at browser-extension shapes, and
  they only see files in the *scanned repository*. An extension sideloaded into
