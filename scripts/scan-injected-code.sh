...
# infected contributor machines and rode into repositories inside otherwise
# normal commits (2025-11 .. 2026-05: vite/webpack/tailwind/postcss configs, and
...
#   --root <dir>                               scan the git repository at <dir>
#                                              instead of the current directory
#
# Exits non-zero if anything is found. See docs/injection-scan.md in
# open-rails/helpers.
...
usage() {
	sed -n '3,29p' "$self" | sed -e 's/^#\{0,1\} \{0,1\}//'
	exit 2
}
...
if [ "$mode" != files ] && ! git rev-parse --show-toplevel >/dev/null 2>&1; then
	echo "injection scan: $(pwd) is not inside a git repository" >&2
	exit 2
fi
...
if [ "$mode" = self-test ]; then
	# Fixtures travel with the scanner, not with the repository being scanned.
	fixtures="$(dirname -- "$self")/testdata/injection-scan"
	[ -d "$fixtures" ] || {
...
if [ "$findings" -gt 0 ]; then
	cat >&2 <<-EOF
		...
		False positive? Tune the specific rule in open-rails/helpers
		scripts/scan-injected-code.sh - do not add a blanket skip.
	EOF
	exit 1
fi
